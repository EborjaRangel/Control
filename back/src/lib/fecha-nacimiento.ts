...
/** Acepta DD/MM/AAAA o ISO; normaliza a YYYY-MM-DD para la base de datos. */
export const fechaNacimientoApiSchema = Yup.string()
  .transform((value) => normalizarFechaNacimientoParaIso(value))
  .required("La fecha de nacimiento es obligatoria")
  .matches(/^\d{4}-\d{2}-\d{2}$/, FECHA_NACIMIENTO_MENSAJE)
  .test("fecha-valida", "La fecha no es válida", (v) =>
    Boolean(v && esFechaIsoValida(v)),
  );
