...
/** Estatus ALTA del Excel (importación): ACTIVO en sistema. */
export function esDirigenteAlta(d: { status: string; activo: boolean }) {
  return d.status === "ACTIVO" && d.activo;
}

/** Estatus BAJA del Excel. */
export function esDirigenteBaja(d: { status: string; activo: boolean }) {
  return d.status === "BAJA" && !d.activo;
}
