...
export default function NominaDirigentePage() {
  ...
  const canAccess = hasAdminPrivileges || canViewOwnDirigente(user, id);
  ...
  const backHref = hasAdminPrivileges ? "/nominas" : `/dirigentes/${user?.dirigenteId}/consultar`;
  ...
  return (
    <div className="space-y-4">
      ...
      <NominaDetalle
        nomina={nomina}
        editing={hasAdminPrivileges}
        saving={saving}
        onSave={hasAdminPrivileges ? handleSave : undefined}
        backHref={backHref}
      />
    </div>
  );
